...
**Mode A: Workspace-based parallel** (`workspace_enabled` + parallel + multi-plan wave)
  ...
  i.2. Reserve human review for explicit exception cases only: ambiguity, intent conflict, missing proof, or quarantined/policy-violating boundary behavior. Those cases must stop before shared planning writes and remain inspectable for follow-up.
