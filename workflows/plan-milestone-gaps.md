<purpose>
Create all phases necessary to close gaps identified by `/bgsd milestone audit`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/bgsd roadmap add` per gap.
</purpose>
...
```
No audit gaps found. Run `/bgsd milestone audit` first.
```
...
PHASES=$(node __OPENCODE_CONFIG__/bgsd-oc/bin/bgsd-tools.cjs plan:phases list)
...
```bash
node __OPENCODE_CONFIG__/bgsd-oc/bin/bgsd-tools.cjs execute:commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/bgsd exec phase {N}` — if plans already exist
...
`/bgsd milestone audit` — re-audit to verify gaps closed
`/bgsd milestone complete {version}` — archive when audit passes
```
