...
```bash
node __OPENCODE_CONFIG__/get-shit-done/bin/gsd-tools.cjs verify:validate health $REPAIR_FLAG
```
...
- [W001] STATE.md references phase 5, but only phases 1-3 exist
  Fix: Run /bgsd-health --repair to regenerate
...
```
---
N issues can be auto-repaired. Run: /bgsd-health --repair
```
</step>
...
```
Would you like to run /bgsd-health --repair to fix N issues automatically?
```
...
```bash
node __OPENCODE_CONFIG__/get-shit-done/bin/gsd-tools.cjs verify:validate health
```
