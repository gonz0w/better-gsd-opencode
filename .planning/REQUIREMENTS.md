...
- [x] **DX-01**: Per-command `--help` support — `COMMAND_HELP` map at module level, `--help` check in router before dispatching, prints usage text to stderr
...
- [x] **DX-03**: Config migration command — `cmdConfigMigrate()` reads existing config, merges with CONFIG_SCHEMA defaults for missing keys, writes back without overwriting existing values
