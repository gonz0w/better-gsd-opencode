...
intent:
  outcome_ids: [DO-06]
  rationale: "Intent evolution tracking enables DO-06 — changes tracked with reasoning across milestones"
...
<objective>
Add intent evolution tracking to INTENT.md — a `<history>` section that logs what changed, why, and in which milestone context.
...
Parsing rules:
- Each `### vX.Y — YYYY-MM-DD` starts a new milestone entry
...
No `<history>` section is written when history is empty — existing INTENT.md files without history remain unchanged.
...
b. If changes detected, create a history entry. Get the current milestone from ROADMAP.md (parse for 🔵 marker to find active milestone version, fallback to "unknown"). Today's date from `new Date().toISOString().split('T')[0]`.
...
d. Auto-generate reason text: `"Updated via intent update {section}"` — this is the default. The user can add `--reason "text"` flag to `intent update` to provide a custom reason.
...
c. Add `'history'` to the `SECTION_ALIASES` array (should be near the top of the file — find `const SECTION_ALIASES = [...]` and add `'history'`).
...
History is optional — `cmdIntentValidate` should NOT fail if `<history>` is missing. If present, validate basic structure:
...
Add as an advisory check (warning, not error) if history has structural issues.
  ...
  <verify>
Run `npm test` — all 340 existing tests pass with zero regressions.

Manual verification:
1. `node bin/gsd-tools.cjs intent show` — compact summary includes "Evolution:" line if history exists, omits it if not
2. `node bin/gsd-tools.cjs intent show history` — renders history section or "(no history recorded)"
3. `node bin/gsd-tools.cjs intent show --raw` — JSON includes `history` array
4. `node bin/gsd-tools.cjs intent update outcomes --value "- DO-07 [P3]: New outcome"` — updates file AND appends history entry
5. `node bin/gsd-tools.cjs intent validate` — passes on INTENT.md without history, passes on INTENT.md with valid history
6. `wc -c bin/gsd-tools.cjs` — within 450KB budget
  </verify>
  <done>
...
a. **Parse INTENT.md without history** — Create INTENT.md without `<history>`, run `intent show --raw`, verify `history` field is `[]` (empty array).

b. **Parse INTENT.md with history** — Create INTENT.md with a `<history>` section containing 2 milestone entries with 3 total changes, run `intent show --raw`, verify all entries and changes parsed correctly.

c. **Update auto-logs history** — Create INTENT.md, run `intent update outcomes --value "- DO-07 [P3]: New test outcome"`, read INTENT.md, verify `<history>` section was appended with an Added entry.

d. **Update with --reason flag** — Run `intent update outcomes --value "- DO-08 [P3]: Another outcome" --reason "Testing reason tracking"`, verify history entry has custom reason.

e. **Show compact includes evolution** — Create INTENT.md with history, run `intent show`, verify output contains "Evolution:" line.

f. **Show history section** — Run `intent show history`, verify output contains "## Intent Evolution" and milestone headers.

g. **Validate accepts history** — Create INTENT.md with valid history, run `intent validate`, verify exit code 0.

h. **Validate works without history** — Create INTENT.md without history, run `intent validate`, verify exit code 0.
...
Follow existing test patterns in the file: use `writeFileSync` to create test INTENT.md, `execSync` to run commands, parse output, assert.
  ...
  <verify>
Run `npm test` — all tests pass including new history tests (expected: 340 + ~8 new = ~348).
Run `node bin/gsd-tools.cjs intent validate` on current project INTENT.md — passes (backward compatible).
...
<verification>
1. Backward compatibility: Existing INTENT.md files without `<history>` work exactly as before — parser returns empty history, generator doesn't write empty section, validate passes, show doesn't mention evolution
2. Forward compatibility: New INTENT.md files get history tracking automatically on first `intent update`
3. Bundle size: `wc -c bin/gsd-tools.cjs` ≤ 450KB (currently 447KB, adding ~100 lines ≈ 3-5KB)
