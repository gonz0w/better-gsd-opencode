...
**Goal**: All 15 feature commands are discoverable and reachable — via slash commands in OpenCode, via --help in the CLI, and via automatic integration in planning/execution workflows
**Depends on**: Phase 2
...
**Success Criteria** (what must be TRUE):
  ...
  2. All 11 previously-unwired commands (session-diff, context-budget, test-run, search-decisions, validate-dependencies, search-lessons, codebase-impact, rollback-info, velocity, trace-requirement, validate-config) are callable as `/gsd-*` slash commands in OpenCode
