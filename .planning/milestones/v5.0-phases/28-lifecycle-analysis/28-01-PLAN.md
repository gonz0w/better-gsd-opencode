...
<task type="auto">
  <name>Task 2: Verify lifecycle detection against event-pipeline project</name>
  <files>src/lib/lifecycle.js</files>
  <action>
Test the lifecycle module against the real event-pipeline project at `/mnt/raid/DEV/event-pipeline/` (per AGENTS.md rule: "Test against real project").

1. First, check if event-pipeline has codebase intel:
   ```
   node bin/gsd-tools.cjs codebase status --raw
   ```
   Run from event-pipeline working directory. If no intel exists, run `codebase analyze` first.
...
2. Load the intel and run buildLifecycleGraph:
   ...
   const cwd = '/mnt/raid/DEV/event-pipeline';
...
3. Verify the output:
   - If Elixir/Phoenix detected in intel.conventions.frameworks → Phoenix detector should activate
   - Migration files in `priv/repo/migrations/` should produce ordered migration nodes
   - `config/config.exs` → `config/runtime.exs` ordering should appear
   - `application.ex` boot node should exist
   - Seeds should depend on last migration
   - Chains should be non-empty and correctly ordered
   - No cycles should be reported

4. Fix any issues found during testing:
   - Path matching not working → adjust regex patterns
   - Missing symmetry → verify post-processing step
   - Incorrect confidence → adjust classification thresholds
   - Missing dedup → verify merge logic

If event-pipeline doesn't have Elixir/Phoenix framework detection in conventions (it might not have run `codebase conventions`), the Phoenix detector won't activate. In that case, verify the generic migration detector works if migration directories exist, and verify the Phoenix detector logic by unit-testing with mock intel data that includes `conventions: { frameworks: [{ framework: 'elixir-phoenix' }] }`.
  </action>
  <verify>
Run the lifecycle detection against event-pipeline — output should have `nodes.length > 0` if the project has migrations/config/boot files. If it's an Elixir project, chains should show migration ordering and config→boot→router flow.

Also test with empty intel: `node -e "const { buildLifecycleGraph } = require('./src/lib/lifecycle'); console.log(JSON.stringify(buildLifecycleGraph({ files: {} }, '.')));"` — should return `{ nodes: [], chains: [], cycles: [], ... }` without errors.
  </verify>
  <done>
Lifecycle detection produces correct results against real project data. Generic migration detector orders numbered/timestamped files correctly. Elixir/Phoenix detector (when applicable) identifies config→boot→migration→seed→router lifecycle. Empty/missing data handled gracefully without crashes.
  </done>
</task>
...
- Tested against real project data (event-pipeline or mock)
