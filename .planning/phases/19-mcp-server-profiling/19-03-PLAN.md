...
Builds on Plan 01's discovery and Plan 02's recommendations. Only modifies `opencode.json` (OpenCode format supports `enabled: false`). Does NOT modify `.mcp.json` (Claude Code format — no standard disable field).
...
Test manually: Copy event-pipeline's opencode.json to a temp dir. Run `mcp-profile --apply` — verify backup created and servers disabled. Run `mcp-profile --restore` — verify original restored.
  </verify>
  <done>
