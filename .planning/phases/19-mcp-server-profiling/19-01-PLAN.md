...
Output: `mcp-profile` CLI command that discovers servers from `.mcp.json` (Claude Code format) and `opencode.json` (OpenCode format), merges them with deduplication, estimates token cost from a known-server database, and outputs a structured JSON report with per-server and total context cost.
</objective>
...
- `.mcp.json`: `{ mcpServers: { name: { command, args, env } } }` (Claude Code format)
...
1. Project `.mcp.json` (Claude Code format): key `mcpServers` → object of `{ name: { command, args?, env? } }`
...
Run `node bin/gsd-tools.cjs mcp-profile --raw` from `/mnt/raid/DEV/event-pipeline` — should list servers from both `.mcp.json` and `opencode.json`.
...
- `node bin/gsd-tools.cjs mcp-profile --raw` from event-pipeline project returns valid JSON with servers from both config files
