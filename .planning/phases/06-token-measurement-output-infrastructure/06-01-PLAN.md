...
**Implementation note for filterFields:** Keep it simple. Dot-notation means split on ".", walk the object. For arrays, recurse into each element. Don't over-engineer — jmespath-level complexity is not needed. The CONTEXT.md says Claude has discretion on whether --fields affects table output too — for now, only filter JSON output (the primary consumer).
