...
7. **CLI integration — codebase lifecycle --raw returns JSON:**
   ...
   it('codebase lifecycle --raw returns valid JSON', () => {
     // Run against the gsd-opencode project itself (which has intel)
...
3. Run `node bin/gsd-tools.cjs codebase lifecycle --raw` from gsd-opencode root — returns valid JSON (even if no lifecycle patterns found in this project, should return success with 0 nodes)
4. Run from event-pipeline: `cd /mnt/raid/DEV/event-pipeline && node /mnt/raid/DEV/gsd-opencode/bin/gsd-tools.cjs codebase lifecycle --raw` — if Elixir project with intel, should detect lifecycle patterns
  </verify>
  <done>
