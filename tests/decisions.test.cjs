...
describe('Phase 128: Agent collaboration decision functions', () => {

  // ─── resolveAgentCapabilityLevel ────────────────────────────────────────────

  describe('resolveAgentCapabilityLevel', () => {
    it('returns HIGH when all 6 tools available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: true, bat: true, gh: true },
      });
      assert.strictEqual(result.value, 'HIGH');
    });

    it('returns HIGH when 5 tools available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: true, bat: true, gh: false },
      });
      assert.strictEqual(result.value, 'HIGH');
    });

    it('returns MEDIUM when 4 tools available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: true, bat: false, gh: false },
      });
      assert.strictEqual(result.value, 'MEDIUM');
    });

    it('returns MEDIUM when 3 tools available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: false, bat: false, gh: false },
      });
      assert.strictEqual(result.value, 'MEDIUM');
    ...
    it('returns MEDIUM when 2 tools available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: false, yq: false, bat: false, gh: false },
      });
      assert.strictEqual(result.value, 'MEDIUM');
    });

    it('returns LOW when 1 tool available', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: false, jq: false, yq: false, bat: false, gh: false },
      });
      assert.strictEqual(result.value, 'LOW');
    });

    it('returns LOW when 0 tools available', () => {
      const result = resolveAgentCapabilityLevel({
    ...
    it('LOW includes warning metadata', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: false, fd: false, jq: false, yq: false, bat: false, gh: false },
      });
      assert.ok(result.metadata, 'Expected metadata on LOW result');
      assert.ok(typeof result.metadata.warning === 'string', 'metadata.warning should be a string');
    });

    it('MEDIUM has no warning metadata', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: false, bat: false, gh: false },
      });
      const hasWarning = result.metadata && result.metadata.warning;
      assert.ok(!hasWarning, 'MEDIUM should not have warning metadata');
    });

    it('HIGH has no warning metadata', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: true, bat: true, gh: true },
      });
      const hasWarning = result.metadata && result.metadata.warning;
      assert.ok(!hasWarning, 'HIGH should not have warning metadata');
    });

    it('satisfies decision contract', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: false, bat: false, gh: false },
      });
      const contract = contractCheck(result, DECISION_CONTRACT, 'agent-capability-level');
      assert.ok(contract.pass, contract.message);
    });

    it('confidence is always HIGH', () => {
      const cases = [
        { ripgrep: true, fd: true, jq: true, yq: true, bat: true, gh: true },
        { ripgrep: true, fd: true, jq: true, yq: false, bat: false, gh: false },
        { ripgrep: false, fd: false, jq: false, yq: false, bat: false, gh: false },
      ];
      for (const ta of cases) {
        const result = resolveAgentCapabilityLevel({ tool_availability: ta });
        assert.strictEqual(result.confidence, 'HIGH', `Expected HIGH confidence for ${JSON.stringify(ta)}`);
      }
    });

    it('rule_id is agent-capability-level', () => {
      const result = resolveAgentCapabilityLevel({
        tool_availability: { ripgrep: true, fd: false, jq: false, yq: false, bat: false, gh: false },
      });
    ...
    it('handles missing tool_availability gracefully', () => {
      const result = resolveAgentCapabilityLevel(null);
      assert.strictEqual(result.value, 'LOW');
      assert.strictEqual(result.rule_id, 'agent-capability-level');
    });

    it('handles empty object', () => {
      const result = resolveAgentCapabilityLevel({ tool_availability: {} });
      assert.strictEqual(result.value, 'LOW');
    });
  });
  ...
  describe('resolvePhaseDependencies', () => {
    it('returns ordered phases for linear dependency chain', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'first' },
          { number: 2, depends_on: [1], name: 'second' },
          { number: 3, depends_on: [2], name: 'third' },
        ],
        tool_availability: {},
      });
      assert.deepStrictEqual(result.value.ordered_phases, [1, 2, 3]);
    });

    it('returns parallel roots at same level', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'first' },
          { number: 2, depends_on: [], name: 'second' },
        ],
        tool_availability: {},
      });
      // Both should appear in ordered_phases (order doesn't matter for parallel roots)
      assert.ok(result.value.ordered_phases.includes(1), 'phase 1 should appear');
      assert.ok(result.value.ordered_phases.includes(2), 'phase 2 should appear');
    });

    it('honors declared dependencies over tool availability', () => {
      // Phase 2 has more tools but depends on phase 1 — must come after
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'foundation', tool_requirements: [] },
          { number: 2, depends_on: [1], name: 'enhanced', tool_requirements: ['ripgrep', 'fd', 'jq'] },
        ],
        tool_availability: { ripgrep: true, fd: true, jq: true },
      });
      const phases = result.value.ordered_phases;
      assert.ok(phases.indexOf(1) < phases.indexOf(2), 'phase 1 must come before phase 2 despite phase 2 having more tools');
    });

    it('uses tool availability as tiebreaker for independent phases', () => {
      // Phase 1 and 2 are independent — phase with available tools should come first
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'alpha', tool_requirements: [] },
          { number: 2, depends_on: [], name: 'beta', tool_requirements: ['ripgrep'] },
        ],
        tool_availability: { ripgrep: true },
      });
      const phases = result.value.ordered_phases;
      // Phase 2 has required tools available — should come before or equal
      // (the tiebreaker places it first if readyB > readyA)
      assert.ok(phases.includes(1) && phases.includes(2), 'both phases should appear');
    });

    it('discovery-before-analysis heuristic', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'analysis-phase' },
          { number: 2, depends_on: [], name: 'discovery-phase' },
        ],
        tool_availability: {},
      });
      const phases = result.value.ordered_phases;
      // discovery-phase (2) should come before analysis-phase (1) due to heuristic
      assert.ok(phases.indexOf(2) < phases.indexOf(1), 'discovery should come before analysis');
    });

    it('detection-before-transform heuristic', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'transform-data' },
          { number: 2, depends_on: [], name: 'detect-structures' },
        ],
        tool_availability: {},
      });
      const phases = result.value.ordered_phases;
      // detect (2) should come before transform (1)
      assert.ok(phases.indexOf(2) < phases.indexOf(1), 'detect should come before transform');
    });

    it('emits warning for phase with missing required tools', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'tools-heavy', tool_requirements: ['ripgrep', 'fd'] },
        ],
        tool_availability: { ripgrep: false, fd: false },
      });
      assert.ok(result.value.warnings.length > 0, 'Expected warning for missing tools');
      assert.ok(result.value.warnings[0].includes('1'), 'Warning should reference phase 1');
    });

    it('confidence HIGH when no warnings', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'simple', tool_requirements: ['ripgrep'] },
        ],
        tool_availability: { ripgrep: true },
      });
      assert.strictEqual(result.confidence, 'HIGH');
    ...
    it('confidence MEDIUM when warnings present', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'needs-tools', tool_requirements: ['ripgrep'] },
        ],
    ...
    it('handles single phase', () => {
      const result = resolvePhaseDependencies({
        phases: [{ number: 5, depends_on: [], name: 'only-phase' }],
        tool_availability: {},
      });
      assert.deepStrictEqual(result.value.ordered_phases, [5]);
    });

    it('handles empty phases array', () => {
      const result = resolvePhaseDependencies({ phases: [], tool_availability: {} });
      assert.deepStrictEqual(result.value.ordered_phases, []);
    ...
    it('handles circular dependency', () => {
      ...
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [2], name: 'phase-a' },
          { number: 2, depends_on: [1], name: 'phase-b' },
        ],
        tool_availability: {},
      });
      // Both phases should still appear in result (cycle broken)
      assert.ok(result.value.ordered_phases.includes(1), 'phase 1 should appear even in cycle');
      assert.ok(result.value.ordered_phases.includes(2), 'phase 2 should appear even in cycle');
      // Should emit warning about cycle
      assert.ok(result.value.warnings.length > 0, 'Expected warning for cycle');
    });

    it('satisfies decision contract', () => {
      const result = resolvePhaseDependencies({
        phases: [{ number: 1, depends_on: [], name: 'test' }],
        tool_availability: {},
      });
      const contract = contractCheck(result, DECISION_CONTRACT, 'phase-dependencies');
      assert.ok(contract.pass, contract.message);
    });

    it('rule_id is phase-dependencies', () => {
      const result = resolvePhaseDependencies({ phases: [], tool_availability: {} });
      assert.strictEqual(result.rule_id, 'phase-dependencies');
    });

    it('ignores completed phases in result ordering', () => {
      // completed phases still appear in ordering (we don't filter them — caller decides)
      // but the function should not error with status: 'complete'
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'done', status: 'complete' },
          { number: 2, depends_on: [1], name: 'pending' },
        ],
        tool_availability: {},
      });
      assert.ok(result.value.ordered_phases.includes(2), 'pending phase should appear');
    });

    it('handles phases with no tool_requirements', () => {
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'no-tools' },
          { number: 2, depends_on: [1], name: 'also-no-tools' },
        ],
        tool_availability: { ripgrep: true },
      });
      assert.deepStrictEqual(result.value.ordered_phases, [1, 2]);
      assert.deepStrictEqual(result.value.warnings, []);
    });

    it('declared deps override discovery heuristic', () => {
      // Even though phase 2 is named 'discovery', it depends on phase 1 (analysis)
      const result = resolvePhaseDependencies({
        phases: [
          { number: 1, depends_on: [], name: 'analysis-base' },
          { number: 2, depends_on: [1], name: 'discovery-dependent' },
        ],
        tool_availability: {},
      });
      const phases = result.value.ordered_phases;
      assert.ok(phases.indexOf(1) < phases.indexOf(2), 'declared dep (1→2) overrides discovery heuristic');
    });

    it('handles null/undefined input gracefully', () => {
      const result = resolvePhaseDependencies(null);
      assert.deepStrictEqual(result.value.ordered_phases, []);
      assert.strictEqual(result.rule_id, 'phase-dependencies');
    });
  });
  ...
  describe('DECISION_REGISTRY integration (Phase 128)', () => {
    it('registry includes agent-capability-level', () => {
      const entry = DECISION_REGISTRY.find(r => r.id === 'agent-capability-level');
      assert.ok(entry, 'agent-capability-level should be in DECISION_REGISTRY');
      assert.strictEqual(typeof entry.resolve, 'function');
    });

    it('registry includes phase-dependencies', () => {
      const entry = DECISION_REGISTRY.find(r => r.id === 'phase-dependencies');
      assert.ok(entry, 'phase-dependencies should be in DECISION_REGISTRY');
      assert.strictEqual(typeof entry.resolve, 'function');
    });

    it('evaluateDecisions fires agent-capability-level when tool_availability in state', () => {
      const results = evaluateDecisions('bgsd-execute-phase', {
        tool_availability: { ripgrep: true, fd: true, jq: true, yq: false, bat: false, gh: false },
      });
      assert.ok(results['agent-capability-level'], 'agent-capability-level should fire');
      assert.ok(['HIGH', 'MEDIUM', 'LOW'].includes(results['agent-capability-level'].value),
        'Expected HIGH, MEDIUM, or LOW');
    });

    it('evaluateDecisions fires phase-dependencies when phases in state', () => {
      ...
      assert.ok(results['phase-dependencies'], 'phase-dependencies should fire');
      assert.ok(Array.isArray(results['phase-dependencies'].value.ordered_phases),
        'ordered_phases should be an array');
    });
  });
});
