...
describe('Group 3: SQLiteDatabase backend (FND-01)', () => {
  ...
  it('getSchemaVersion() returns 1 after initial migration', () => {
    const version = db.getSchemaVersion();
    assert.strictEqual(version, 1, 'Schema version should be 1 after initial V1 migration');
  });
...
describe('Group 6: Schema migration (FND-02)', () => {
  ...
  it('fresh DB starts at version 0 before getDb(), version 1 after', () => {
    // We verify indirectly: a fresh .planning/ dir, calling getDb() results in version 1
    tempDir = makeTempDir('bgsd-migration-');
    fs.mkdirSync(path.join(tempDir, '.planning'), { recursive: true });
    const db = getDb(tempDir);
    assert.strictEqual(db.getSchemaVersion(), 1, 'Schema should be version 1 after initial migration');
  });
  ...
  it('calling getDb() again on same DB is idempotent — version stays at 1', () => {
    tempDir = makeTempDir('bgsd-idempotent-');
    fs.mkdirSync(path.join(tempDir, '.planning'), { recursive: true });

    // First call
    const db1 = getDb(tempDir);
    assert.strictEqual(db1.getSchemaVersion(), 1);

    // Second call — same instance (singleton)
    const db2 = getDb(tempDir);
    assert.strictEqual(db1, db2, 'Should be same instance');
    assert.strictEqual(db2.getSchemaVersion(), 1, 'Version should still be 1 (migrations not re-run)');
  });
});
...
describe('Group 7: Migration failure → delete-and-rebuild', () => {
  ...
  it('corrupted .cache.db is detected, deleted, and rebuilt without error', () => {
    ...
    if (db.backend === 'sqlite') {
      assert.strictEqual(db.getSchemaVersion(), 1, 'Rebuilt DB should have schema version 1');
