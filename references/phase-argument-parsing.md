...
```bash
PHASE_INFO=$(node __OPENCODE_CONFIG__/get-shit-done/bin/gsd-tools.cjs plan:find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node __OPENCODE_CONFIG__/get-shit-done/bin/gsd-tools.cjs plan:roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node __OPENCODE_CONFIG__/get-shit-done/bin/gsd-tools.cjs plan:find-phase "${PHASE}" | jq -r '.directory')
```
